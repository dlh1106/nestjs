...
export class DebugExplorer {
  ...
  constructor(
    ...
    for (const wrapper of instanceWrappers.filter(
      (wrap: InstanceWrapper) => !wrap.isNotMetatype,
    )) {
      ...
      const metadata = this.reflector.get<DebugOptions | undefined>(
        DEBUG_METADATA,
        metatype,
      );
      if (!metadata) {
  ...
  private applyDecorator(metatype: Metatype, metadata: DebugOptions): void {
    ...
    for (const meta of instanceMetatypes) {
      if (
        ...
        this.exclude.has(meta.name) ||
        metadata.exclude?.includes(meta)
      ) {
        continue;
      }
    ...
    const imports =
      this.reflector.get<Type[] | undefined>('imports', metatype) || [];
